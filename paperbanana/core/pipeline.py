...
class PaperBananaPipeline:
    ...
    async def generate(self, input: GenerationInput) -> GenerationOutput:
        ...
        if mode == "vanilla":
            ...
            return await self._run_fast(input)
        elif mode == "full":
    ...
    async def _run_fast(self, input: GenerationInput) -> GenerationOutput:
        """Fast mode: Planner-only -> Visualizer -> Critic loop.

        Skips Retriever and Stylist for maximum speed. Guidelines are
        injected directly into the Planner prompt.
        """
        guidelines = self._get_guidelines(input.diagram_type)

        logger.info("Phase 1: Planning (fast mode, no retrieval)")
        ...
        logger.info("[Planner] done", seconds=round(planning_seconds, 1))

        if self.settings.save_iterations:
            save_json(
                {"mode": "fast", "description": description, "planner_ratio": planner_ratio},
                self._run_dir / "planning.json",
            )
    ...
    async def _run_full(self, input: GenerationInput) -> GenerationOutput:
        ...
        description, planner_ratio = await self.planner.run(
            ...
            diagram_type=input.diagram_type,
            supported_ratios=getattr(self.visualizer.image_gen, "supported_ratios", None),
            guidelines=guidelines if self.settings.skip_stylist else None,
        )
        planning_seconds = time.perf_counter() - planning_start
        logger.info(
            "[Planner] done",
        ...
        # Step 3: Stylist — optimize description aesthetics (or skip if merged into Planner)
        if self.settings.skip_stylist:
            logger.info("Phase 1: Stylist skipped (merged into Planner)")
        ...
        for i in range(total_iters):
            ...
            # Early exit: score threshold (skip remaining iterations if quality is high enough)
            score_threshold = self.settings.critic_score_threshold
            if score_threshold > 0 and critique.score is not None:
                if critique.score >= score_threshold:
                    logger.info(
                        "Score threshold met, skipping remaining iterations",
                        iteration=i + 1,
